...
private static Pusher pusher = null;
	...
	public static void main(String[] args)throws Exception{
		...
		settings = ApipManagerSettings.loadMySettings(sid,br);
		...
		Rewarder.checkRewarderParams(sid,params,esClient,jedisPool,br);
		...
		counter  = new Counter(settings,params, null, symKey);
		counter.run();

//		startOrderScan(setter,params);
//		startMempoolScan();
//		startPusher(esClient);
		...
		while(true) {
			...
			switch (choice) {
				...
				case 4 -> manageReward(sid, params,esClient, jedisPool, br);
	...
	public static void checkSwapIndices(ElasticsearchClient esClient) throws IOException {
		...
		nameMappingList.put(server.Settings.addSidBriefToName(sid,SWAP_STATE), SwapStateData.swapStateJsonStr);
		EsTools.checkEsIndices(StartAPIP.esClient,nameMappingList);
...
//	}
	...
	private static void setParamsToRedis(Configure configure, byte[] symKey, Service myService, ApipParams myServiceParams) {
		try(Jedis jedis = jedisPool.getResource()) {
			...
			jedis.hset(server.Settings.addSidBriefToName(sid,SETTINGS),ES_ACCOUNT_ID, settings.getEsAccountId());
			jedis.hset(server.Settings.addSidBriefToName(sid,SETTINGS),WINDOW_TIME, String.valueOf(settings.getWindowTime()));
			...
			jedis.hset(server.Settings.addSidBriefToName(sid,SETTINGS),INIT_SYM_KEY_CIPHER,symKeyCipher);
...
//	}
	...
	private static void manageReward(String sid, Params params, ElasticsearchClient esClient, JedisPool jedisPool, BufferedReader br) {
		RewardManager rewardManager = new RewardManager(sid, params.getAccount(),null,esClient, jedisPool, br);
	...
	private static void writeParamsToRedis() {
		...
		String paramsOnChainKey = Settings.addSidBriefToName(sid,PARAMS);
		String settingsKey = Settings.addSidBriefToName(sid,SETTINGS);
		try(Jedis jedis = jedisPool.getResource()) {
			jedis.hset(paramsOnChainKey, ACCOUNT, params.getAccount());
			jedis.hset(paramsOnChainKey, CURRENCY, params.getCurrency());
			jedis.hset(paramsOnChainKey, URL_HEAD, params.getUrlHead());
			if (params.getMinPayment() != null)
				jedis.hset(paramsOnChainKey, MIN_PAYMENT, params.getMinPayment());
			if (params.getPricePerKBytes() != null)
				jedis.hset(paramsOnChainKey, PRICE_PER_K_BYTES, params.getPricePerKBytes());
			if (params.getPricePerRequest() != null)
				jedis.hset(paramsOnChainKey, PRICE_PER_REQUEST, params.getPricePerRequest());
			if (params.getSessionDays() != null)
				jedis.hset(paramsOnChainKey, SESSION_DAYS, params.getSessionDays());
			if (params.getConsumeViaShare() != null)
				jedis.hset(paramsOnChainKey, CONSUME_VIA_SHARE, params.getConsumeViaShare());
			if (params.getOrderViaShare() != null)
				jedis.hset(paramsOnChainKey, ORDER_VIA_SHARE, params.getOrderViaShare());
